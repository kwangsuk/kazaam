...
// go test "github.com/qntfy/kazaam/v4/transform" -coverprofile cover.out
...
func getJSONRaw(data []byte, path string, pathRequired bool, keySeparator string) ([]byte, error) {
	...
	return getProcessedJSONRaw(data, jsonPathParams.getJsonPath(), pathRequired, jsonPathParams, keySeparator)
}
func getProcessedJSONRaw(data []byte, path string, pathRequired bool, params *JSONPathParameters, keySeparator string) ([]byte, error) {
			...
			if arrayKeyStr == "*" {
				...
				if newPath != "" {
					for i, value := range results {
						intermediate, err := getProcessedJSONRaw(value, newPath, pathRequired, params, keySeparator)
						if err == jsonparser.KeyPathNotFoundError {
							if pathRequired {
								return nil, NonExistentPath
							}
